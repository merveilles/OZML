...
public class vp_FileDialog : EditorWindow
{
	...
	void OnGUI()
	{
		...
		{
			GUI.enabled = false;
			pos = new Vector2(Screen.width - 400, 20);
			GUI.Label(new Rect(pos.x, pos.y, 300, 30), "NOTE: For your presets to work in a webplayer build, \nthey must be saved under a folder named 'Resources'.", m_NoteStyle);
			GUI.enabled = true;
		}	
